...
angular.module("controllers")
	.controller('pacienteCtrl', ['$scope','$routeParams', function($scope,$routeParams) {
        ...
        $scope.getPastNotes=function(lastNumbers){
            ...
            $rootScope.evolucionDB.allDocs({include_docs: true, descending: true}, function(err, doc) {
               console.log(doc)
              });
            //GET PAST NOTES FROM PARSE
        ...
        $scope.savePacient=function(invalid){
            if(invalid)
                ...
                //Chek if user Profile Picture is available
                var profilePicture=$("#userImageUpload")[0].files;
                console.log(profilePicture)
                if(profilePicture.length>0){
                    console.log("image")
                    var parseFile=new Parse.File(profilePicture[0].name, profilePicture[0])
                    currentPacient.set("IMAGEN_PERFIL",parseFile)
                }
